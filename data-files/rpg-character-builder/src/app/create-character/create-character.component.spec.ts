...
describe('CreateCharacterComponent', () => {
  ...
  it('should generate a random character ID between 1 and 1000 with no decimal places', () => {
    const character = component.addCharacter({ value: { name: 'Test', gender: 'Male', class: 'Warrior' } });
    expect(character.id).toBeGreaterThan(0);
    expect(character.id).toBeLessThanOrEqual(1000);
    expect(Number.isInteger(character.id)).toBe(true);
  });
